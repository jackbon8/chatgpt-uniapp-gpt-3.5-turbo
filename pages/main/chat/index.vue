			...
			<view  class="flex-column-start" v-for="(x,i) in msgList" :key="i">
				...
				<view v-if="!x.my" class="flex-row-start margin-left margin-top one-show" >
					<view class="chat-img flex-row-center">
						<image style="height: 75rpx;width: 75rpx;" src="/static/image/549.jpg" mode="aspectFit"></image>
					</view>
					<view  class="flex"  style="width: 500rpx;">
						<view class="margin-left padding-chat flex-column-start" style="border-radius: 35rpx;background-color: #f9f9f9;">
							...
							<view class="flex-column-start" v-if="x.type==1" style="color: #2fa39b;">
								...
								<text @click="answer(index)" style="margin-top: 10rpx;" 
								...
								<text class="" style="margin-top: 10rpx;color: #007AFF;">请尝试更换问题描述</text>
							</view>
							<!-- 消息模板 =>多个答案 -->
							...
							<view class="flex-column-start" v-if="x.type==0">
								...
								<text @click="answer(index)" style="margin-top: 10rpx;color: #2fa39b;" 
								v-for="(item,index) in x.questionList" :key="index" >{{item}}</text>
								<view class="flex-row-start  padding-top-sm">
									<text class="my-neirong-sm">没有你要的答案?</text>
	...
	export default {	
		...
		data() {
			return {
				...
				msgList:[{my:false,msg:"你好！",
				type:1,questionList:["我可以提供问题解决思路","我还可以连续对话"]}],
				msg:"",
				go:0,
				srcollHeight:0,
				chainContent: ''
		...
		methods: {
			...
			msgKf(x){		
				...
				uni.request({
					url: '/api/aichat/gpt',
					data: {content:_this.chainContent, token:token},
					method: 'POST',
					success(res) {
						if(res.statusCode == 200){
							if(res.data.code == 1){
								// 取消loading
								_this.msgLoad=false
								_this.chainContent +=  res.data.data;
								let answerString = res.data.data
								_this.msgList.push({my:false,msg:answerString,type:-1})
								_this.msgGo()
							} else {
				...
				// 	this.msgList.push({my:false,msg:"您的提问："+x+'问题答案： 123123123123123123',type:-1})
